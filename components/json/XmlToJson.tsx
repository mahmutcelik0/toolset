...
export default function XmlToJson() {
    ...
    const [status, setStatus] = useState<Status>({
        type: "idle",
        msg: "XML girip Dönüştür butonuna tıklayın"
    })
    const [toast, setToast] = useState<string>("")
    const debounce = useRef<ReturnType<typeof setTimeout> | undefined>(undefined)
    const isFirstRender = useRef(true)

    // XML to JSON dönüşümü
    const convert = useCallback((val: string, ind: number) => {
        if (!val.trim()) {
            setOutput("")
            setStatus({
                type: "idle",
                msg: "XML girin veya yapıştırın"
            })
            return
        }
        ...
        try {
            const parser = new XMLParser({
                ...
                trimValues: true,
                isArray: (tagName: string, jPath: string | any, isLeafNode: boolean, isAttribute: boolean) => {
                    // 'rol' tag'ını dizi olarak döndür
                    // jPath artık string veya MatcherView olabilir
                    if (tagName === 'rol') return true
                    return false
                }
            })
            ...
            setStatus({
                type: "ok",
                msg: "✅ XML başarıyla JSON'a dönüştürüldü",
            ...
            setStatus({
                type: "error",
                msg: `❌ Geçersiz XML: ${error.message}`
            })
        }
    }, [])
