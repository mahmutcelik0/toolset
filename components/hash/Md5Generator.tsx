...
const SAMPLE_TEXT = `Merhaba Dünya! Bu bir MD5 test metnidir.
Hello World! This is an MD5 test text.
1234567890 !@#$%^&*()`
...
export default function Md5Generator() {
    ...
    const [status, setStatus] = useState<Status>({
        type: "idle",
        msg: "Metin girip MD5 oluştur'a tıklayın"
    })
    ...
    // MD5 hash fonksiyonu (md5 kütüphanesi kullanarak)
    const generateMD5 = useCallback((text: string, upperCase: boolean): string => {
        if (!text) return ""
        const hash = md5(text)
    ...
    // Hash işlemi
    const processHash = useCallback((text: string, upperCase: boolean) => {
        if (!text.trim()) {
            setOutput("")
            setStatus({
                type: "idle",
        ...
        try {
            const hash = generateMD5(text, upperCase)
            setOutput(hash)

            setStatus({
                type: "ok",
                msg: "✅ MD5 hash oluşturuldu",
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-4 p-3 bg-[#1a1d27] border border-white/8 rounded-lg">
                ...
                <span className="text-xs text-white/30 ml-2">MD5: 128-bit (32 karakter)</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="flex flex-col bg-[#1a1d27] border border-white/8 rounded-xl overflow-hidden">
                    <div className="flex items-center justify-between px-3 py-2 bg-[#22263a] border-b border-white/8 shrink-0">
                        <span className="text-xs font-medium tracking-widest text-white/30 uppercase">MD5 Hash</span>
            ...
            <div className="bg-[#1a1d27] border border-white/8 rounded-xl overflow-hidden">
                <div className="px-3 py-2 bg-[#22263a] border-b border-white/8">
                    <span className="text-xs font-medium tracking-widest text-white/30 uppercase">MD5 Bilgisi</span>
                </div>
